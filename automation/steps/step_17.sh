#!/usr/bin/env bash
# DB RU automation step 17: Start node1 database instance
# Generated production-ready wrapper for AWX/AAP workflow execution.
# Mapping rule: Workflow Extra Vars step_id="17" -> this script.
...
STEP_NAME="${STEP_NAME:-Step 17 - start node1 instance}"
ru_require_mode
ru_step_begin

case "${RUN_MODE}" in
  mock)
    ru_mock "would start node1 instance ${NODE1_INSTANCE:-<inst>}"
    ;;
  check)
  ...
  real)
    ...
    ru_run srvctl start instance -d "${DB_UNIQUE_NAME}" -i "${NODE1_INSTANCE}"
    ;;
esac
